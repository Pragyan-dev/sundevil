...
export function FloatingChatWidget() {
  ...
  return (
    <div className="floating-chat-shell pointer-events-none fixed bottom-3 right-3 z-50 left-3 flex flex-col items-end gap-3 sm:bottom-6 sm:right-6 sm:left-auto sm:max-w-[calc(100vw-3rem)]">
      {open ? (
        <>
          <button
            type="button"
            onClick={() => setOpen(false)}
            className="floating-chat-backdrop pointer-events-auto fixed inset-0 z-0 bg-[rgba(17,12,8,0.18)] backdrop-blur-[2px] sm:hidden"
            aria-label="Dismiss chat overlay"
          />
          <div className="floating-chat-panel pointer-events-auto relative z-10 flex max-h-[min(44rem,calc(100dvh-1.5rem-env(safe-area-inset-top)-env(safe-area-inset-bottom)))] w-full flex-col overflow-hidden rounded-[1.7rem] border border-[rgba(0,0,0,0.1)] bg-[#f9f3ea]/96 shadow-[0_28px_80px_rgba(18,12,8,0.28)] backdrop-blur sm:w-[min(26rem,calc(100vw-3rem))] sm:max-h-[min(44rem,calc(100dvh-6rem))]">
            <div className="floating-chat-header flex items-start justify-between gap-3 border-b border-[rgba(140,29,64,0.12)] bg-[rgba(255,255,255,0.82)] px-4 py-3">
      ...
      {!open ? (
        <button
          ...
          className="floating-chat-launcher pointer-events-auto inline-flex items-center gap-3 rounded-full border border-[rgba(0,0,0,0.12)] bg-[rgba(255,255,255,0.96)] px-4 py-3 text-left shadow-[0_20px_60px_rgba(16,11,7,0.22)] backdrop-blur transition hover:-translate-y-0.5 hover:border-[var(--asu-gold)] hover:bg-white"
          aria-expanded={open}
